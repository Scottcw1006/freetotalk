...
**跑很久的探索腳本會撞到指令逾時（本環境的 Bash 工具上限 120 秒）。** 一邊捲畫面一邊 dump 的腳本很容易超過。
做法：把它丟到背景跑（工具的 `run_in_background`），或把單次執行的步數砍到能在 100 秒內結束。
**確認方式**：`time <你的腳本>`，看它會不會超過 100 秒。
...
在每次冷啟動後內容可能不同。**還原後比對雜湊時要把這一類檔案分開看**，否則會誤判成還原失敗。
...
同一個座標就落在系統的導覽手勢區 —— 點下去會離開受測 app，而後續的輸入全部灌進別的 app。**這一輪踩了兩次。**
...
**不要用「我記得剛才開了抽屜/子畫面」去推現在按返回會退到哪裡。** 那個記憶會過期（有東西自己關掉了、或上一個點擊其實沒生效），
而一旦推錯，那一次返回就把你送出 app 了。**每次按返回之前先問一次 `mCurrentFocus` 與 `mInputShown`，按完再問一次。**
