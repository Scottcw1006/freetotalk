...
*最後確認：2026-09-10（快照 → 推測試檔 → 大量操作 → 刪測試檔 → 還原偏好 → 雜湊與 baseline 完全相同）*
...
**確認方式**（判斷焦點還在不在）：送一個字元進去，再逐碼位讀欄位（第 5 節）；欄位有變就代表焦點還在。TAB 之後再送一個字元，欄位不再變，就是焦點真的走了。
...
*最後確認：2026-09-10（照這個流程編譯成功並跑完一次基準 + 三次突變 + 一支自寫的 main 探針，專案原始碼的 mtime 全程未變）*
